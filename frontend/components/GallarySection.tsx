...
export default function GallerySection() {
  ...
  return (
    <div className="px-8 mt-12 pt-8 sm:mt-16 sm:pt-12 lg:mt-10 lg:pt-16 bg-gradient-to-b from-white-900 to-stone-200/80 ">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sm:gap-8">
        <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
          <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-18 lg:h-18 bg-stone-100 rounded-xl flex items-center justify-center shrink-0">
            <Images className="w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 text-cyan-950" />
          </div>
        ...
        <Button
          asChild
          className="bg-white text-cyan-950 hover:bg-stone-50 h-14 sm:h-16 px-8 sm:px-10 text-lg sm:text-xl rounded-full font-semibold w-full sm:w-auto shadow-md transition-transform"
        >
      ...
      {/* Displaying Gallery Images in 3x1 Grid with Gap */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-10 pb-15">
        {candidateData.gallery.images.slice(0, 3).map((image) => (
