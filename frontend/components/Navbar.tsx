    ...
    <header className="sticky top-0 z-50 w-full border-b border-stone-200/50 bg-white/95 backdrop-blur-md shadow-sm">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative w-14 h-14 sm:w-16 sm:h-16 shrink-0 overflow-hidden rounded-full ring-2 ring-green-900/20 group-hover:ring-green-900/40 transition-all group-hover:scale-105 shadow-sm">
            <Image
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-serif text-sm lg:text-xl sm:text-lg font-bold tracking-tight text-cyan-950 leading-tight truncate">
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                {link.label}
                {active && (
                  <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-cyan-950 rounded-full" />
                )}
              </Link>
            )
          })}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative group">
            ...
            >
              <Globe className="w-4 h-4 text-stone-600 group-hover:text-green-800" />
              <span className="text-sm font-semibold text-stone-700 group-hover:text-green-900">
                {language === "en" ? "EN" : "BN"}
              </span>
            </button>
              ...
              <>
                ...
                <div className="absolute top-full right-0 mt-2 bg-white border border-stone-200 rounded-xl shadow-lg z-40 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  >
                    ...
                    {language === "bn" && (
                      <span className="ml-auto text-green-900">✓</span>
                    )}
                  </button>
                </div>
              </>
          ...
          <Button
            asChild
            variant="outline"
            className="hidden lg:flex border-green-800 text-green-900 hover:bg-teal-50 rounded-full bg-transparent"
          >
          ...
          <button
            ...
            className={`lg:hidden p-2 rounded-lg transition-colors relative z-50 ${
              isMobileMenuOpen
                ? "text-white bg-cyan-950/70"
                : "text-stone-700 hover:text-green-900 hover:bg-teal-50"
            }`}
              ...
              <div className="flex flex-col gap-4">
                  ...
                  return (
                    <Link
                      ...
                      className={`px-4 py-3 rounded-lg text-base font-medium transition-colors ${
                        active
                          ? "text-cyan-950 bg-teal-50 font-semibold"
                          : "text-stone-700 hover:text-green-900 hover:bg-green-50/50"
                      }`}
                    >
                      {link.label}
                    </Link>
                ...
                <div className="flex flex-col gap-3 pt-4 border-t border-stone-200">
                  <Button
                    asChild
                    variant="outline"
                    className="w-full border-green-800 text-green-900 hover:bg-green-50 rounded-full bg-transparent"
                    onClick={closeMobileMenu}
                  >
                    <Link href="/form/volunteer">{t.navbar.volunteer}</Link>
                  </Button>
